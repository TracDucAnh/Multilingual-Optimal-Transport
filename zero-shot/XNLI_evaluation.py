...
Scoring Strategy — Log-prob:
    Mỗi sample được score bằng cách tính mean NLL của 3 candidate labels:
...
def evaluate(
    ...
    """
    Chạy toàn bộ log-prob evaluation trên XNLI.
    ...
    Log-prob scoring:
        ...
        - pred = argmin(mean_nll) trên C candidates của cùng sample_id
    ...
    print(f"\n{'═'*58}")
    print(f"  XNLI Zero-shot Results — {MODEL_NAME}")
    print(f"{'═'*58}")
    ...
    # ── Confusion matrix per language (optional debug) ────────────────────
    # pred_label x gold_label counts across all languages
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="XNLI zero-shot evaluation — Llama-3-8B vanilla baseline"
    )
    parser.add_argument(
